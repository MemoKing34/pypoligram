...
class OnUserStatus:
	def on_user_status(
		self: Union["OnUserStatus", PFilter, Filter, None] = None,
		...
		Parameters:
			...
			filters (:obj:`~pyrogram.filters`, *optional*):
				Pass one or more filters to allow only a subset of user status changes to be passed
				in your function.
		...
		def decorator(func: Callable) -> Callable:
			...
			if isinstance(self, pypoligram.ClientManager):
				self.add_handler(pyrogram.handlers.UserStatusHandler(func, filters), client_filters or ALL, group)

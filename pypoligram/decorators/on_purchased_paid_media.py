...
class OnPurchasedPaidMedia:
	def on_purchased_paid_media(
		self: Union["OnPurchasedPaidMedia", PFilter, Filter, None] = None,
		...
		Parameters:
			...
			filters (:obj:`~pyrogram.filters`, *optional*):
				Pass one or more filters to allow only a subset of purchased paid media
				to be passed in your function.
		...
		def decorator(func: Callable) -> Callable:
			...
			if isinstance(self, pypoligram.ClientManager):
				self.add_handler(pyrogram.handlers.PurchasedPaidMediaHandler(func, filters), client_filters or ALL, group)
				...
				func.handlers.append(
					(
						pyrogram.handlers.PurchasedPaidMediaHandler(func, filters),
